		...
		size_t count, loff_t *offset)
{
	...
	while (count > I2C_FIFO) {
		ret = i2c_master_send(pn544_dev->client, sent_tmp + addr, I2C_FIFO);
	...
	ret = i2c_master_send(pn544_dev->client, sent_tmp + addr, count);
		pr_info("%s: i2c_master_send returned %d\n", __func__, ret);
	num+=ret;
	    pr_info("%s : Status %d \n", __func__,num);
	    
	 if(num != inall) {
	 	pr_info("%s :num diff from count! \n", __func__);
	  	goto fail; 			
	 }
	return num;

fail:	
	pr_err("%s: i2c_master_recv returned fail %d\n", __func__, ret);
	ret = -EIO;
	return ret;
}

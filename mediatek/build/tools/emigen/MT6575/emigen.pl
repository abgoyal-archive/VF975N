...
my $PROJECT               = $ARGV[3];
my $JRD_RAM_SIZE = $ARGV[4];
print "qiujuan test.. memory ram size = $JRD_RAM_SIZE \n";
...
if ($JRD_RAM_SIZE eq "1G")
{
  if ($os eq "windows")
  {
    $CUSTOM_EMI_H =~ s/custom_MemoryDevice_1G.h$/output\\custom_emi\.h/i;
    $CUSTOM_EMI_C =~ s/custom_MemoryDevice_1G.h$/output\\custom_emi\.c/i;
    `mkdir output` unless (-d "output");
  }
  elsif ($os eq "linux")
  {
    $CUSTOM_EMI_H =~ s/custom_MemoryDevice_1G.h$/custom_emi\.h/i;
    $CUSTOM_EMI_C =~ s/inc\/custom_MemoryDevice_1G.h$/custom_emi\.c/i;
    $INFO_TAG     =~ s/inc\/custom_MemoryDevice_1G.h$/MTK_Loader_Info\.tag/i;
  }
}else
{
  if ($os eq "windows")

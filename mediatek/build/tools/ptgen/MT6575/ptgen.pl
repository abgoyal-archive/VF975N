...
my $JRD_RAM_SIZE = $ARGV[10];
print "qiujuan test .. JRD_RAM_SIZE= $JRD_RAM_SIZE \n";
...
#my $CUSTOM_MEMORYDEVICE_H_NAME  = "mediatek/custom/$PROJECT/preloader/inc/$MEMORY_DEVICE_NAME";
...
if ($JRD_RAM_SIZE eq "1G")
{
$CUSTOM_MEMORYDEVICE_H_NAME  = "mediatek/custom/$PROJECT/preloader/inc/custom_MemoryDevice_1G.h";
print "qiujuan test .. use 1G DEVICE HEAD\n";
}
else
{
$CUSTOM_MEMORYDEVICE_H_NAME  = "mediatek/custom/$PROJECT/preloader/inc/custom_MemoryDevice.h";
print "qiujuan test .. use 512M DEVICE HEAD\n";
}
#Set SCAT_NAME
...
print "SHARE_SDCARD=$MTK_SHARED_SDCARD\n";
...
#			print "$'\n";
	...
	foreach $part_num (@lines) {
		if(exists $REGION_TABLE{$part_num}){
			...
			print "Chose region layout: $part_num, $REGION_TABLE{$part_num}{BOOT1} + $REGION_TABLE{$part_num}{BOOT2} + $REGION_TABLE{$part_num}{RPMB}=$cur{USER}=$cur_user\n";
			if ($cur > $MAX_address) {
				$MAX_address = $cur;
			}
...
sub InitAlians(){
	...
	$kernel_alias{"CUSTPACK"}="custpack";
	$kernel_alias{"MOBILE_INFO"}="mobile_info";
...
sub GenScatFile ()
{
	...
	# for mini software build, do not cancel userdata downloading
	# otherwise modem meta connect fails
	$TARGET_BUILD_MMITEST = $ENV{TARGET_BUILD_MMITEST};
	if ($TARGET_BUILD_MMITEST eq "true" && $PARTITION_FIELD[$iter] =~ /USRDATA/) {
		$temp = "";
	}
if($EMMC_SUPPORT eq "yes" && $RESERVED_FIELD[$iter] == 1 && $PLATFORM eq "MT6575"){
			$temp .= "RSV_";
	}
...
my @BR = (
	...
	["mediatek/misc/EBR2", [[0x83,0x0,0x0],
						[0x05,0x0,0x0]]],
	["mediatek/misc/EBR3", [[0x83,0x0,0x0],
						[0x05,0x0,0x0]]],
	["mediatek/misc/EBR4", [[0x83,0x0,0x0]]]
);
...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		...
		if($PARTITION_FIELD[$iter] eq "ANDROID"){
			$p8_start_block = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
			$p8_size_block =  ($SIZE_FIELD_KB[$iter]-1024)*1024/512;
		}
	}
	#MBR
	...
	print "P8 start is $p8_start_block\n";
	print "P8 size is $p8_size_block\n";
	#EBR1
	...
	$BR[3][1][1][1] = $p8_start_block - 0x20 - 0x20;
	$BR[3][1][1][2] = $p8_size_block + 0x20;
    
    #EBR4
	$BR[4][1][0][1] = 0x20;
	$BR[4][1][0][2] = $p8_size_block; 
...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		...
		if($PARTITION_FIELD[$iter] eq "EBR2"||$PARTITION_FIELD[$iter] eq "EBR3"||$PARTITION_FIELD[$iter] eq "EBR4"){
		...
		if($PARTITION_FIELD[$iter] eq "CACHE" || $PARTITION_FIELD[$iter] eq "USRDATA" || $PARTITION_FIELD[$iter] eq "SEC_RO" || $PARTITION_FIELD[$iter] eq "ANDROID" 
			|| $PARTITION_FIELD[$iter] eq "CUSTPACK" || $PARTITION_FIELD[$iter] eq "MOBILE_INFO"){
	...
	for($iter=1;$iter<=$p_index_count;$iter++){
		if($iter<=4){
			...
			if($br_count == 2){
				$BR[$br_count][1][0][1] = $start_block[$iter] - $br_start[$br_count];
				$BR[$br_count][1][0][2] = $size_block[$iter];
				$BR[$br_count][1][1][1] = $start_block[$iter + 1] - $br_start[$br_count];
				$BR[$br_count][1][1][2] = $size_block[$iter + 1];
			}	
			$br_count++;		
...
sub GenPartSizeFile
{
	...
	for($index=0;$index < $total_rows;$index++){
		if($PARTITION_FIELD[$index] eq "SEC_RO")
	     ...
	     if($PARTITION_FIELD[$index] eq "CUSTPACK")
	     {		# Reserve 1 MB for "encrypt phone" function. It needs 16k for CRYPT_FOOTER.MBR include it.
			# 1MB for sec info check in with CR ALPS00073371
		 	$temp=$SIZE_FIELD_KB[$index]/1024-1-1;
			print EMMC_PART_SIZE_LOCATION "BOARD_CUSTPACKIMAGE_PARTITION_SIZE:=$temp". "M\n" ;
	      }

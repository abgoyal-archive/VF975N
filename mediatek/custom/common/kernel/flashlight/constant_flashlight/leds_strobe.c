...
*****************************************************************************/
#if 1
#define GPIO_CAMERA_FLASH_MODE GPIO48
...
static int constant_flashlight_ioctl(MUINT32 cmd, MUINT32 arg)
{
    ...
    switch(cmd)
    {
		...
		case FLASHLIGHT_TORCH_SELECT:
			...
			if (arg && strobe_width){
				printk("@@@@@@FLASHLIGHT_TORCH_SELECT@111111111@@@@@\n");
				TORCH_select();
				g_strobe_On = TRUE;
			}
			else {
				printk("@@@@@@FLASHLIGHT_TORCH_SELECT@222222222@@@@@\n");
				if(FL_Disable()) {

...
struct bma_i2c_data {
	...
	BMA_POWERMODE_ENUM power_mode;
	BMA_RANGE_ENUM range;
	BMA_DATARATE_ENUM datarate;
	u16 sensitivity;/* sensitivity = 2^bitnum/range [+/-2G = 4; +/-4G = 8; +/-8G = 16 ] */
...
* Note: When select BMA2X2, it support BMA222E, BMA250E, BMA255, BMA280 automatically.
*/
static struct i2c_board_info __initdata bma_i2c_info = {
    I2C_BOARD_INFO(BMA_DEV_NAME, BMA_I2C_ADDRESS(BMA222,PIN_LOW))
};
...
static void bma_power(struct acc_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, BMA_DEV_NAME))
			{
...
static int bma_read_raw_data(struct i2c_client *client, s16 data[BMA_AXES_NUM])
{
	...
	if (priv->sensor_type == BMA220_TYPE) {/* BMA220 */
		if ((err = bma_i2c_read_block(client, BMA220_REG_DATAXLOW, buf, 3))){
		...
		{
			GSE_LOG("[%s][8bit raw data][%08X %08X %08X] => [%5d %5d %5d]\n", priv->sensor_name, (s16)buf[BMA_AXIS_X], (s16)buf[BMA_AXIS_Y], (s16)buf[BMA_AXIS_Z],
		                               (s16)buf[BMA_AXIS_X], (s16)buf[BMA_AXIS_Y], (s16)buf[BMA_AXIS_Z]);
			GSE_LOG("[%s][6bit raw data][%08X %08X %08X] => [%5d %5d %5d]\n", priv->sensor_name, data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z],
		                               data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z]);
		}
	}else if (priv->sensor_type == BMA222_TYPE) {/* BMA222 */
		if ((err = bma_i2c_read_block(client, BMA222_REG_DATAXLOW, buf, 6))){
			GSE_ERR("[%s]read raw data failed, err = %d\n", priv->sensor_name, err);
			return err;
		}
		...
		if ((err = bma_i2c_read_block(client, BMA2X2_REG_DATAXLOW, buf, 6))){
			GSE_ERR("[%s]read raw data failed, err = %d\n", priv->sensor_name, err);
			return err;
		}
		data[BMA_AXIS_X] = (s16)buf[BMA_AXIS_X*2 + 1];
		...
		if ((err = bma_i2c_read_block(client, BMA250_REG_DATAXLOW, buf_tmp, 6))){
			GSE_ERR("[%s]read raw data failed, err = %d\n", priv->sensor_name, err);
			return err;
		}
		data[BMA_AXIS_X] = (s16)(((u16)buf_tmp[1]) << 8 | buf_tmp[0]) >> 6;
		data[BMA_AXIS_Y] = (s16)(((u16)buf_tmp[3]) << 8 | buf_tmp[2]) >> 6;
		data[BMA_AXIS_Z] = (s16)(((u16)buf_tmp[5]) << 8 | buf_tmp[4]) >> 6;
		
		if(atomic_read(&priv->trace) & ADX_TRC_RAWDATA)
		{
			GSE_LOG("[%s][10bit raw data][%08X %08X %08X] => [%5d %5d %5d]\n", priv->sensor_name, data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z],
		                               data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z]);
		}
	}else if (priv->sensor_type == BMA250E_TYPE) {/* BMA250E */
		u8 buf_tmp[BMA_DATA_LEN] = {0};
		if ((err = bma_i2c_read_block(client, BMA2X2_REG_DATAXLOW, buf_tmp, 6))){
			GSE_ERR("[%s]read raw data failed, err = %d\n", priv->sensor_name, err);
			return err;
		}
		data[BMA_AXIS_X] = (s16)(((u16)buf_tmp[1]) << 8 | buf_tmp[0]) >> 6;
		...
		data[BMA_AXIS_X] = (s16)(((u16)buf_tmp[1]) << 8 | buf_tmp[0]) >> 4;
		data[BMA_AXIS_Y] = (s16)(((u16)buf_tmp[3]) << 8 | buf_tmp[2]) >> 4;
		data[BMA_AXIS_Z] = (s16)(((u16)buf_tmp[5]) << 8 | buf_tmp[4]) >> 4;
		
		if(atomic_read(&priv->trace) & ADX_TRC_RAWDATA)
		{
			GSE_LOG("[%s][12bit raw data][%08X %08X %08X] => [%5d %5d %5d]\n", priv->sensor_name, data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z],
		                               data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z]);
		}
		...
		data[BMA_AXIS_X] = (s16)(((u16)buf_tmp[1]) << 8 | buf_tmp[0]) >> 2;
		data[BMA_AXIS_Y] = (s16)(((u16)buf_tmp[3]) << 8 | buf_tmp[2]) >> 2;
		data[BMA_AXIS_Z] = (s16)(((u16)buf_tmp[5]) << 8 | buf_tmp[4]) >> 2;
		
		if(atomic_read(&priv->trace) & ADX_TRC_RAWDATA)
		{
			GSE_LOG("[%s][14bit raw data][%08X %08X %08X] => [%5d %5d %5d]\n", priv->sensor_name, data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z],
		                               data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z]);
		}
	}
...
*/
	if(atomic_read(&priv->filter))
	{
		if(atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend))
		{
			...
			{                
				priv->fir.raw[priv->fir.num][BMA_AXIS_X] = data[BMA_AXIS_X];
				priv->fir.raw[priv->fir.num][BMA_AXIS_Y] = data[BMA_AXIS_Y];
				priv->fir.raw[priv->fir.num][BMA_AXIS_Z] = data[BMA_AXIS_Z];
				...
				if(atomic_read(&priv->trace) & ADX_TRC_FILTER)
				{
					GSE_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n", priv->fir.num,
						priv->fir.raw[priv->fir.num][BMA_AXIS_X], priv->fir.raw[priv->fir.num][BMA_AXIS_Y], priv->fir.raw[priv->fir.num][BMA_AXIS_Z],
						priv->fir.sum[BMA_AXIS_X], priv->fir.sum[BMA_AXIS_Y], priv->fir.sum[BMA_AXIS_Z]);
				}
				priv->fir.num++;
				priv->fir.idx++;
			}
			else
			{
				...
				priv->fir.sum[BMA_AXIS_X] -= priv->fir.raw[idx][BMA_AXIS_X];
				priv->fir.sum[BMA_AXIS_Y] -= priv->fir.raw[idx][BMA_AXIS_Y];
				priv->fir.sum[BMA_AXIS_Z] -= priv->fir.raw[idx][BMA_AXIS_Z];
				priv->fir.raw[idx][BMA_AXIS_X] = data[BMA_AXIS_X];
				priv->fir.raw[idx][BMA_AXIS_Y] = data[BMA_AXIS_Y];
				priv->fir.raw[idx][BMA_AXIS_Z] = data[BMA_AXIS_Z];
				priv->fir.sum[BMA_AXIS_X] += data[BMA_AXIS_X];
				priv->fir.sum[BMA_AXIS_Y] += data[BMA_AXIS_Y];
				priv->fir.sum[BMA_AXIS_Z] += data[BMA_AXIS_Z];
				priv->fir.idx++;
				data[BMA_AXIS_X] = priv->fir.sum[BMA_AXIS_X]/firlen;
				data[BMA_AXIS_Y] = priv->fir.sum[BMA_AXIS_Y]/firlen;
				data[BMA_AXIS_Z] = priv->fir.sum[BMA_AXIS_Z]/firlen;
				if(atomic_read(&priv->trace) & ADX_TRC_FILTER)
				{
					GSE_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d] : [%5d %5d %5d]\n", idx,
					priv->fir.raw[idx][BMA_AXIS_X], priv->fir.raw[idx][BMA_AXIS_Y], priv->fir.raw[idx][BMA_AXIS_Z],
					priv->fir.sum[BMA_AXIS_X], priv->fir.sum[BMA_AXIS_Y], priv->fir.sum[BMA_AXIS_Z],
					data[BMA_AXIS_X], data[BMA_AXIS_Y], data[BMA_AXIS_Z]);
				}
			}
...
static int bma_get_hw_offset(struct i2c_client *client, s8 offset[BMA_AXES_NUM + 1])
{    
	int err = 0;
...
static int bma_set_hw_offset(struct i2c_client *client, s8 offset[BMA_AXES_NUM + 1])
{    
	int err = 0;
...
#else
	if(err = bma_set_hw_offset(client, obj->offset))		
	{	
...
static int bma_read_calibration(struct i2c_client *client, int act[BMA_AXES_NUM], int raw[BMA_AXES_NUM])
{  
	/* raw: the raw calibration data, unmapped; act: the actual calibration data, mapped */
	struct bma_i2c_data *obj = i2c_get_clientdata(client);
	int err = 0;
	int mul;
	...
	#else
	if(err = bma_get_hw_offset(client, obj->offset))		
	{	
		GSE_ERR("read hw offset failed, %d\n", err);	
		return err;	
	}
	...
	raw[BMA_AXIS_X] = obj->offset[BMA_AXIS_X]*mul + obj->cali_sw[BMA_AXIS_X];
	raw[BMA_AXIS_Y] = obj->offset[BMA_AXIS_Y]*mul + obj->cali_sw[BMA_AXIS_Y];
	raw[BMA_AXIS_Z] = obj->offset[BMA_AXIS_Z]*mul + obj->cali_sw[BMA_AXIS_Z];

	act[obj->cvt.map[BMA_AXIS_X]] = obj->cvt.sign[BMA_AXIS_X]*raw[BMA_AXIS_X];
	act[obj->cvt.map[BMA_AXIS_Y]] = obj->cvt.sign[BMA_AXIS_Y]*raw[BMA_AXIS_Y];
	act[obj->cvt.map[BMA_AXIS_Z]] = obj->cvt.sign[BMA_AXIS_Z]*raw[BMA_AXIS_Z];
	                       
	return err;
}

static int bma_write_calibration(struct i2c_client *client, int dat[BMA_AXES_NUM])
{
	...
	if((err = bma_read_calibration(client, cali, raw)))	/*offset will be updated in obj->offset */
	{ 
		GSE_ERR("read offset fail, %d\n", err);
		return err;
	}

	GSE_LOG("OLD OFFSET: unmapped raw offset(%+3d %+3d %+3d), unmapped hw offset(%+3d %+3d %+3d), unmapped cali_sw (%+3d %+3d %+3d)\n", 
		raw[BMA_AXIS_X], raw[BMA_AXIS_Y], raw[BMA_AXIS_Z],
		obj->offset[BMA_AXIS_X], obj->offset[BMA_AXIS_Y], obj->offset[BMA_AXIS_Z],
		obj->cali_sw[BMA_AXIS_X], obj->cali_sw[BMA_AXIS_Y], obj->cali_sw[BMA_AXIS_Z]);
	...
	obj->cali_sw[BMA_AXIS_X] = obj->cvt.sign[BMA_AXIS_X]*(cali[obj->cvt.map[BMA_AXIS_X]]);
	obj->cali_sw[BMA_AXIS_Y] = obj->cvt.sign[BMA_AXIS_Y]*(cali[obj->cvt.map[BMA_AXIS_Y]]);
	obj->cali_sw[BMA_AXIS_Z] = obj->cvt.sign[BMA_AXIS_Z]*(cali[obj->cvt.map[BMA_AXIS_Z]]);
#else
	int divisor = 1; /* divisor = sensor sensitivity / offset sensitivity */
	obj->offset[BMA_AXIS_X] = (s8)(obj->cvt.sign[BMA_AXIS_X]*(cali[obj->cvt.map[BMA_AXIS_X]])/(divisor));
	obj->offset[BMA_AXIS_Y] = (s8)(obj->cvt.sign[BMA_AXIS_Y]*(cali[obj->cvt.map[BMA_AXIS_Y]])/(divisor));
	obj->offset[BMA_AXIS_Z] = (s8)(obj->cvt.sign[BMA_AXIS_Z]*(cali[obj->cvt.map[BMA_AXIS_Z]])/(divisor));

	/*convert software calibration using standard calibration*/
	obj->cali_sw[BMA_AXIS_X] = obj->cvt.sign[BMA_AXIS_X]*(cali[obj->cvt.map[BMA_AXIS_X]])%(divisor);
	obj->cali_sw[BMA_AXIS_Y] = obj->cvt.sign[BMA_AXIS_Y]*(cali[obj->cvt.map[BMA_AXIS_Y]])%(divisor);
	obj->cali_sw[BMA_AXIS_Z] = obj->cvt.sign[BMA_AXIS_Z]*(cali[obj->cvt.map[BMA_AXIS_Z]])%(divisor);

	GSE_LOG("NEW OFFSET: unmapped raw offset(%+3d %+3d %+3d), unmapped hw offset(%+3d %+3d %+3d), unmapped cali_sw(%+3d %+3d %+3d)\n", 
...
/* set power mode */
static int bma_set_powermode(struct i2c_client *client, BMA_POWERMODE_ENUM power_mode)
{
	struct bma_i2c_data *obj = i2c_get_clientdata(client);
	...
	if (obj->sensor_type == BMA220_TYPE) {/* BMA220 */
		...
		err = bma_i2c_read_block(client, BMA222_EN_LOW_POWER__REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA222_EN_LOW_POWER, 0);/* disable low power mode */
		data = BMA_SET_BITSLICE(data, BMA222_EN_SUSPEND, actual_power_mode);
		err += bma_i2c_write_block(client, BMA222_EN_LOW_POWER__REG, &data, 1);
	}else if ((obj->sensor_type == BMA222E_TYPE) || (obj->sensor_type == BMA250E_TYPE)\
	|| (obj->sensor_type == BMA255_TYPE) || (obj->sensor_type == BMA280_TYPE)) {/* BMA222E, BMA250E, BMA255, BMA280 */
		u8 data1 = 0, data2 = 0;
		if (power_mode == BMA_SUSPEND_MODE){
		...
		err = bma_i2c_read_block(client, BMA2X2_MODE_CTRL_REG, &data1, 1);
		err += bma_i2c_read_block(client, BMA2X2_LOW_NOISE_CTRL_REG, &data2, 1);
		
		switch (actual_power_mode) {
		case BMA2X2_MODE_NORMAL:
			data1 = BMA_SET_BITSLICE(data1, BMA2X2_MODE_CTRL, BMA2X2_MODE_NORMAL);
			data2 = BMA_SET_BITSLICE(data2, BMA2X2_LOW_POWER_MODE, 0);
			err += bma_i2c_write_block(client, BMA2X2_MODE_CTRL_REG, &data1, 1);
			mdelay(1);
			err += bma_i2c_write_block(client, BMA2X2_LOW_NOISE_CTRL_REG, &data2, 1);
		break;
		case BMA2X2_MODE_SUSPEND:
			data1 = BMA_SET_BITSLICE(data1, BMA2X2_MODE_CTRL, BMA2X2_MODE_SUSPEND);
			data2 = BMA_SET_BITSLICE(data2, BMA2X2_LOW_POWER_MODE, 0);
			err += bma_i2c_write_block(client, BMA2X2_LOW_NOISE_CTRL_REG, &data2, 1);
			mdelay(1);
			err += bma_i2c_write_block(client, BMA2X2_MODE_CTRL_REG, &data1, 1);
		...
		err = bma_i2c_read_block(client, BMA250_EN_LOW_POWER__REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA250_EN_LOW_POWER, 0);/* disable low power mode */
		data = BMA_SET_BITSLICE(data, BMA250_EN_SUSPEND, actual_power_mode);
		err += bma_i2c_write_block(client, BMA250_EN_LOW_POWER__REG, &data, 1);
	}
...
static int bma_set_range(struct i2c_client *client, BMA_RANGE_ENUM range)
{
	struct bma_i2c_data *obj = i2c_get_clientdata(client);
	...
	if (obj->sensor_type == BMA220_TYPE) {/* BMA220 */
		...
		if(err < 0)
			GSE_ERR("set range failed, err = %d, sensor name = %s\n", err, obj->sensor_name);
		else {
			obj->range = range;
			/* bitnum: 6bit */
			switch (range) {
		...
		err = bma_i2c_read_block(client, BMA222_RANGE_SEL__REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA222_RANGE_SEL, actual_range);
		err += bma_i2c_write_block(client, BMA222_RANGE_SEL__REG, &data, 1);
		...
		err = bma_i2c_read_block(client, BMA2X2_RANGE_SEL_REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA2X2_RANGE_SEL, actual_range);
		err += bma_i2c_write_block(client, BMA2X2_RANGE_SEL_REG, &data, 1);
		...
		err = bma_i2c_read_block(client, BMA250_RANGE_SEL__REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA250_RANGE_SEL, actual_range);
		err += bma_i2c_write_block(client, BMA250_RANGE_SEL__REG, &data, 1);
...
static int bma_set_datarate(struct i2c_client *client, BMA_DATARATE_ENUM datarate)
{
	struct bma_i2c_data *obj = i2c_get_clientdata(client);
	...
	if (obj->sensor_type == BMA220_TYPE) {/* BMA220 */
		if ((datarate == BMA_DATARATE_10HZ) || (datarate == BMA_DATARATE_25HZ)\
		...
		err = bma_i2c_read_block(client, BMA220_SC_FILT_CONFIG__REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA220_SC_FILT_CONFIG, bandwidth);	  
		err += bma_i2c_write_block(client, BMA220_SC_FILT_CONFIG__REG, &data, 1);
	}else if (obj->sensor_type == BMA222_TYPE) {/* BMA222 */
		...
		err = bma_i2c_read_block(client, BMA222_BANDWIDTH__REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA222_BANDWIDTH, bandwidth);	  
		err += bma_i2c_write_block(client, BMA222_BANDWIDTH__REG, &data, 1);
	}else if ((obj->sensor_type == BMA222E_TYPE) || (obj->sensor_type == BMA250E_TYPE)\
	|| (obj->sensor_type == BMA255_TYPE) || (obj->sensor_type == BMA280_TYPE)) {/* BMA222E, BMA250E, BMA255, BMA280 */
		if (datarate == BMA_DATARATE_10HZ)
		...
		err = bma_i2c_read_block(client, BMA2X2_BANDWIDTH__REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA2X2_BANDWIDTH, bandwidth);	  
		err += bma_i2c_write_block(client, BMA2X2_BANDWIDTH__REG, &data, 1);
	}else if (obj->sensor_type == BMA250_TYPE) {/* BMA250 */
		if (datarate == BMA_DATARATE_10HZ)
		...
		err = bma_i2c_read_block(client, BMA250_BANDWIDTH__REG, &data, 1);
		data = BMA_SET_BITSLICE(data, BMA250_BANDWIDTH, bandwidth);
		err += bma_i2c_write_block(client, BMA250_BANDWIDTH__REG, &data, 1);
	}
	
	if(err < 0)
		GSE_ERR("set bandwidth failed, err = %d,sensor type = %d\n", err, obj->sensor_type);
	else
...
{
#ifdef CONFIG_BMA_LOWPASS
	struct bma_i2c_data *obj = (struct bma_i2c_data*)i2c_get_clientdata(client);
#endif
	int err = 0;
	GSE_FUN(f);
	...
	err = bma_set_datarate(client, BMA_DATARATE_10HZ);
	if(err < 0)
	{
	...
	err = bma_set_range(client, BMA_RANGE_2G);
	if(err < 0 ) 
	{
	...
	err = bma_set_powermode(client, BMA_SUSPEND_MODE);
	if(err < 0)
	{
...
*/
static int bma_read_sensor_data(struct i2c_client *client, char *buf, int bufsize)
{
	struct bma_i2c_data *obj = (struct bma_i2c_data*)i2c_get_clientdata(client);
	...
	if ((err = bma_read_raw_data(client, databuf)))
	{        
	...
	else
	{
		...
		/* remap coordinate */
		acc[obj->cvt.map[BMA_AXIS_X]] = obj->cvt.sign[BMA_AXIS_X]*databuf[BMA_AXIS_X];
		acc[obj->cvt.map[BMA_AXIS_Y]] = obj->cvt.sign[BMA_AXIS_Y]*databuf[BMA_AXIS_Y];
		acc[obj->cvt.map[BMA_AXIS_Z]] = obj->cvt.sign[BMA_AXIS_Z]*databuf[BMA_AXIS_Z];

		/* convert: LSB -> m/s^2 */
		acc[BMA_AXIS_X] = acc[BMA_AXIS_X] * GRAVITY_EARTH_1000 / obj->sensitivity;
		acc[BMA_AXIS_Y] = acc[BMA_AXIS_Y] * GRAVITY_EARTH_1000 / obj->sensitivity;
		acc[BMA_AXIS_Z] = acc[BMA_AXIS_Z] * GRAVITY_EARTH_1000 / obj->sensitivity;		
		
		sprintf(buf, "%04x %04x %04x", acc[BMA_AXIS_X], acc[BMA_AXIS_Y], acc[BMA_AXIS_Z]);
		printk(KERN_ALERT "x=%04x y=%04x z=%04x",acc[BMA_AXIS_X], acc[BMA_AXIS_Y], acc[BMA_AXIS_Z]);
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
	...
	if((err = bma_read_calibration(obj->client, act, raw)))
	{
		return -EINVAL;
	}
	else
	{    
		...
		len += snprintf(buf+len, PAGE_SIZE-len, "[ALL]unmapped(%+3d, %+3d, %+3d), mapped(%+3d, %+3d, %+3d)\n", 
			raw[BMA_AXIS_X], raw[BMA_AXIS_Y], raw[BMA_AXIS_Z],
...
*unit:mapped LSB 
*Example:
*	if only force +1 LSB to android z-axis via s/w calibration, type command in terminal:
...
static ssize_t store_cali_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		if((err = bma_reset_calibration(obj->client)))
		{
			GSE_ERR("reset offset err = %d\n", err);
		}	
	}
	else if(BMA_AXES_NUM == sscanf(buf, "0x%02X 0x%02X 0x%02X", &dat[BMA_AXIS_X], &dat[BMA_AXIS_Y], &dat[BMA_AXIS_Z]))
	{
		if((err = bma_write_calibration(obj->client, dat)))
		{
...
static ssize_t show_firlen_value(struct device_driver *ddri, char *buf)
{
	...
	if(atomic_read(&obj->firlen))
	{
		...
		for(idx = 0; idx < len; idx++)
		{
			GSE_LOG("[%5d %5d %5d]\n", obj->fir.raw[idx][BMA_AXIS_X], obj->fir.raw[idx][BMA_AXIS_Y], obj->fir.raw[idx][BMA_AXIS_Z]);
		}
		
		GSE_LOG("sum = [%5d %5d %5d]\n", obj->fir.sum[BMA_AXIS_X], obj->fir.sum[BMA_AXIS_Y], obj->fir.sum[BMA_AXIS_Z]);
		GSE_LOG("avg = [%5d %5d %5d]\n", obj->fir.sum[BMA_AXIS_X]/len, obj->fir.sum[BMA_AXIS_Y]/len, obj->fir.sum[BMA_AXIS_Z]/len);
	}
...
static ssize_t store_power_mode_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if(err == 0)
	{
		if((err = bma_set_powermode(obj->client,(BMA_POWERMODE_ENUM)(!!(power_mode)))))
		{
			return err;
		}
		return count;
...
static ssize_t store_range_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if(err == 0)
	{
		if((range == BMA_RANGE_2G)||(range == BMA_RANGE_4G)||(range == BMA_RANGE_8G))
		{
			if((err = bma_set_range(obj->client, range)))
			{
				return err;
			}
			return count;
...
static ssize_t store_datarate_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		if((datarate == BMA_DATARATE_10HZ)||(datarate == BMA_DATARATE_25HZ)\
		||(datarate == BMA_DATARATE_50HZ)||(datarate == BMA_DATARATE_100HZ)||(datarate == BMA_DATARATE_200HZ))
		{
			if((err = bma_set_datarate(obj->client, datarate)))
			{
				return err;
			}
			return count;
...
static DRIVER_ATTR(range,      S_IWUSR | S_IRUGO, show_range_value,         store_range_value);
static DRIVER_ATTR(datarate,   S_IWUSR | S_IRUGO, show_datarate_value,      store_datarate_value);
...
static struct driver_attribute *bma_attr_list[] = {
	...
	&driver_attr_trace,        /* trace flag */
	&driver_attr_status,       /* get hw configuration */
	&driver_attr_powermode,    /* get power mode */
	&driver_attr_range,        /* get range */
	&driver_attr_datarate,     /* get data rate */
};
...
static int bma_create_attr(struct device_driver *driver) 
{
	...
	{
		if((err = driver_create_file(driver, bma_attr_list[idx])))
		{            
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{
				value = *(int *)buff_in;/* value:[0--->suspend, 1--->normal] */
				GSE_LOG("sensor enable/disable command: %s\n", value?"enable":"disable");
				
				if((err = bma_set_powermode(priv->client, (BMA_POWERMODE_ENUM)(!!value))))
				{
					GSE_ERR("set power mode failed, err = %d\n", err);
				}
			}
...

{
	...
	switch(cmd)
	{
		case GSENSOR_IOCTL_INIT:
			...
			if((err = bma_set_powermode(client, BMA_NORMAL_MODE)))
			{
				err = -EFAULT;
				break;
			}
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			...
			if ((err = bma_read_raw_data(client, raw_data)))
			{
				err = -EFAULT;
				break;
			}
			sprintf(strbuf, "%04x %04x %04x", raw_data[BMA_AXIS_X], raw_data[BMA_AXIS_X], raw_data[BMA_AXIS_X]);
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			{
				/* convert: m/s^2 -> LSB */
				cali[BMA_AXIS_X] = sensor_data.x * obj->sensitivity / GRAVITY_EARTH_1000;
				cali[BMA_AXIS_Y] = sensor_data.y * obj->sensitivity / GRAVITY_EARTH_1000;
				cali[BMA_AXIS_Z] = sensor_data.z * obj->sensitivity / GRAVITY_EARTH_1000;			  
		...
		case GSENSOR_IOCTL_GET_CALI:
			...
			if((err = bma_read_calibration(client, cali, raw_offset)))
			{
				break;
			}

			sensor_data.x = cali[BMA_AXIS_X] * GRAVITY_EARTH_1000 / obj->sensitivity;
			sensor_data.y = cali[BMA_AXIS_Y] * GRAVITY_EARTH_1000 / obj->sensitivity;
			sensor_data.z = cali[BMA_AXIS_Z] * GRAVITY_EARTH_1000 / obj->sensitivity;
...
static int bma_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	{   
		...
		if((err = bma_set_powermode(obj->client, BMA_SUSPEND_MODE)))
		{
...
static int bma_resume(struct i2c_client *client)
{
	...
	bma_power(obj->hw, 1);
	if((err = bma_init_client(client, 0)))
	{
		GSE_ERR("initialize client failed, err = %d\n", err);
		return err;        
	}

	if((err = bma_set_powermode(obj->client, BMA_NORMAL_MODE)))
	{
		GSE_ERR("bma set normal mode failed, err = %d\n", err);
		return;
	}
...
static int bma_i2c_detect(struct i2c_client *client, struct i2c_board_info *info) 
{    
	strcpy(info->type, BMA_DEV_NAME);
	return 0;
}

static int bma_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = bma_init_client(client, 1)))
	{
		goto exit_init_client_failed;
	}

	if((err = misc_register(&bma_device)))
	{
		GSE_ERR("misc device register failed, err = %d\n", err);
		goto exit_misc_device_register_failed;
	}

	if((err = bma_create_attr(&bma_gsensor_driver.driver)))
	{
...
static int bma_i2c_remove(struct i2c_client *client)
{
	...
	if((err = bma_delete_attr(&bma_gsensor_driver.driver)))
	{
		GSE_ERR("bma_delete_attr failed, err = %d\n", err);
	}
	
	if((err = misc_deregister(&bma_device)))
	{
...
static int __init bma_init(void)
{
	...
	{
		GSE_ERR("register gsensor platform driver failed,return -ENODEV\n");
		return -ENODEV;
	}
	return 0;
}

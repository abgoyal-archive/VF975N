...
*/
  //      if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV3640_YUV,currSensorName)))
	...
	if (0 == pinSetIdx || 2 == pinSetIdx) {
		if(mt_set_gpio_mode(GPIO214, GPIO_MODE_00))
			PK_DBG("hujl@debug:[CAMERA SENSOR AF ] set gpio mode failed!! \n");
		if(mt_set_gpio_dir(GPIO214,GPIO_DIR_OUT))
			PK_DBG("hujl@debug:[CAMERA SENSOR] set gpio dir failed!! \n");
		if(mt_set_gpio_out(GPIO214,GPIO_OUT_ONE))
			PK_DBG("hujl@debug:[CAMERA SENSOR] set gpio failed!! \n");
	}
    }
    else {//power OFF
	...
	if (0 == pinSetIdx || 2 == pinSetIdx) {
		if(mt_set_gpio_mode(GPIO214, GPIO_MODE_00))
			PK_DBG("[CAMERA SENSOR AF ] set gpio mode failed!! \n");
		if(mt_set_gpio_dir(GPIO214,GPIO_DIR_OUT))
			PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");
		if(mt_set_gpio_out(GPIO214,GPIO_OUT_ZERO))

...
unsigned int mt6577_get_bus_freq(void)
{
    return 12000000;
}
#else
...
#if 0 /* MT6577 only 6 DMA channel for UART */
...
/*---------------------------------------------------------------------------*/
static void dump_reg(struct mtk_uart *uart, const char* caller)
{
#ifdef ENABLE_DEBUG
    u32 base = uart->base;
...
static int __init mtk_uart_console_setup(struct console *co, char *options)
{
    ...
    printk(KERN_ALERT DBG_TAG "mtk console uart setting: (%d, %d, %d, %lu, %lu)\n", 
...
static irqreturn_t mtk_vfifo_irq_handler(int irq, void *dev_id)
{
	...
	reg_sync_writel(0 , VFF_INT_FLAG(vfifo->base));
...
{
#ifndef CONFIG_MT6577_FPGA
    struct mtk_uart_setting *setting = uart->setting;
    ...
    MSG(FUC, "%s(%d) => up \n", __func__, uart->poweron_count); 
#endif /* End of CONFIG_MT6577_FPGA */
}
/*---------------------------------------------------------------------------*/
...
/* For Dual Talk */
#ifdef UART_EN_MTK_DT_SUPPORT
#if !defined(EVDO_DT_SUPPORT)

#define UART_DT_PORT    (0)
...
#ifdef UART_EN_MTK_DT_SUPPORT
#if !defined(EVDO_DT_SUPPORT)
    request_wakeup_md_timeout(UART_DT_PORT, uart->nport);
#endif
#endif
...
static int mtk_uart_startup(struct uart_port *port)
{
    ...
    printk("mtk_get_bus_freq() return %d\r\n",mt6577_get_bus_freq());  //Fixme
	...
	switch(uart->nport){
	    case 0:
		disable_dpidle_by_bit(MT65XX_PDN_PERI_UART0);
	        break;
	    case 1:
...
/*---------------------------------------------------------------------------*/
void mt6577_uart_update_sysclk(void)
{
	int i;
	unsigned long flags;
	struct mtk_uart *uart;
	...
	for (i = 0; i < UART_NR; i++) {
		...
		port = &uart->port;
		baud = uart->baudrate;
		port->uartclk  		= mt6577_get_bus_freq()*1000/4;		//Fixme
		uart->sysclk        = mt6577_get_bus_freq()*1000/4;		//Fixme
...
EXPORT_SYMBOL(mt6577_uart_update_sysclk);
/*---------------------------------------------------------------------------*/
...
{ 
    ...
    if (flags & UART_CONFIG_TYPE) {
        ...
        port->type = PORT_MT6575; 	//FIXME Need modify serial_core.h
    }
}
/*---------------------------------------------------------------------------*/
    ...
    struct serial_struct *ser)
{
    ...
    if (ser->type != PORT_UNKNOWN && ser->type != PORT_MT6575) //FIXME Need modify serial_core.h
        ret = -EINVAL;
    if (ser->irq != port->irq)
        ret = -EINVAL;
...
static int mtk_uart_init_ports(void)
{
    ...
    unsigned long base;
#if defined(CONFIG_MTK_SERIAL_MODEM_TEST)
    //#define HW_MISC     (CONFIG_BASE+0x0020)    // mt6577 does NOT has this register
...
#else
	uart->port.uartclk  = mt6577_get_bus_freq()*1000/4;
	uart->sysclk        = mt6577_get_bus_freq()*1000/4;
#endif
        pll_for_uart = mt6577_get_bus_freq();
...
#if defined(CONFIG_MTK_SERIAL_MODEM_TEST)
        if (modem_uart[i]) {            
            //u32 dat = UART_READ32(HW_MISC); // mt6577 does NOT has this register
        ...
        mtk_uart_disable_intrs(uart, UART_IER_ALL_INTS);
#if 0
        mtk_irq_set_sens(uart->setting->irq_num, uart->setting->irq_sen);
        mtk_irq_set_polarity(uart->setting->irq_num, MT65xx_POLARITY_LOW);
#endif
        if (uart->setting->irq_num == MT65xx_EDGE_SENSITIVE) {
    ...
    /*NOTICE: for enabling modem test, UART4 needs to be disabled. Howerver, if CONFIG_MTK_SERIAL_CONSOLE
              ...
              function.*/
    //printk("HW_MISC: 0x%08X\n", UART_READ32(HW_MISC)); // mt6577 does NOT has this register
#endif 
    return 0;
}
/*---------------------------------------------------------------------------*/

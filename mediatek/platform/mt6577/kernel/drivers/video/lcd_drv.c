...
//added by xiewei for lcd esd check --begin
...
//					OUTREG16(&LCD_REG->START, 0x8000);
    ...
    while(_IsEngineBusy()) {
	    udelay(50);//sleep 1ms
		wait_time++;
		if(wait_time>2000){ //timeout  100ms
			LCD_REG_DSI_DC tmp_reg;
			printk("[WARNING] Wait for LCD engine not busy timeout!!!\n");
			LCD_DumpRegisters();
			tmp_reg = LCD_REG->DS_DSI_CON;
...
//added by xiewei for lcd esd check --end
...
LCD_STATUS jrd_esd_LCD_WaitForNotBusy(void)
{
...
//    _WaitForEngineNotBusy();
    ...
    else if (id < LCD_LAYER_NUM)
    {
		...
		if (LCD_LAYER_FORMAT_YUY2 == format) {
			format = LCD_LAYER_FORMAT_YUYV422;
			ctrl.SWP = true;
		}
		else// if (LCD_LAYER_FORMAT_YUYV422 == format)
			ctrl.SWP = false;
...
//added by xiewei for lcd sd check --begin
LCD_STATUS jrd_esd_LCD_StartTransfer(BOOL blocking)
{
    LCD_REG_SIZE mainWndSize = _lcdContext.roiWndSize;
...
//added by xiewei for lcd sd check --end
...
BOOL LCD_esd_check(void)
{
	...
	LCD_CHECK_RET(jrd_esd_LCD_StartTransfer(TRUE));

 ...
 * Description:
 * ------------
 *   This Module defines functions of 6329 fuel gauge
 *
 * Author:
 * -------
 * -------
...
int gCurrent_rst = 0;
int gSW_rst_err = 0;
int gFG_columb_err = 0;
int gSW_rst_err_columb = 0;
...
void get_hw_chip_diff_trim_value(void)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Chip_Trim] %d,%d,%d\n", 
        chip_diff_trim_value_en, chip_diff_trim_value_4_0, chip_diff_trim_value);
}
...
//Auto adjust value
    ...
    //add by chenfei for the error of MT6329 Fuelgauge Bug 396149
    printk("CHENFEI FGADC gFG_columb RAW data is %d \n",dvalue_CAR);
    if(gSW_rst_err == 1)
	{
		dvalue_CAR = gFG_columb;
	}
    else if(dvalue_CAR > gFG_columb && upmu_is_chr_det() != KAL_TRUE) //discharging and the gFG_columb only decrease
	{
		dvalue_CAR = gFG_columb;
		gFG_columb_err = 1;
	}
    else if((gFG_columb - dvalue_CAR) >= 10 && upmu_is_chr_det() != KAL_TRUE)	// 10 means 3.6A working current for 10s
	{
		dvalue_CAR = gFG_columb;
		gFG_columb_err = 1;
	}
...
//calculate the real world data    
    ...
    //add by feichen@tcl.com for PR 390175 patch

    if(dvalue == 0)
	{
        ret=pmic_config_interface(0xC8, 0x1, 0xFF, 0x0);//Enable VA2
        ret=pmic_bank1_config_interface(0x15, 0x0, 0xFF, 0x0);// Enable FGADC clock for digital
        ret=pmic_bank1_config_interface(0x69, 0x28, 0xFF, 0x0);// Set current mode, auto-calibration mode and 32KHz clock source
        ret=pmic_bank1_config_interface(0x69, 0x29, 0xFF, 0x0);// Enable FGADC
	}
    //end add by feichen@tcl.com for PR 390175 patch
    ...
    if((gFG_Is_Charging == KAL_TRUE)&&(upmu_is_chr_det() == KAL_FALSE))
        {
		gCurrent_rst = 1;
		gFG_Is_Charging == KAL_FALSE;
		dvalue = gFG_current;
...
void fgauge_Normal_Mode_Work(void)
{
    int i=0;
        ...
        //get_hw_chip_diff_trim_value(); //move to init
...
{    
    ...
    //add by chenfei
    if(upmu_is_chr_det() != KAL_TRUE && gFG_columb < 0)
    {
	gSW_rst_err_columb += (gFG_columb%15);	//15mAH is 1% of 1500mAH
	printk("CHENFEI FGADC SW Reset gSW_rst_err_columb = %d",gSW_rst_err_columb);
	if(gSW_rst_err_columb <= -15)
	{
		gSW_rst_err_columb = 0;
		gFG_DOD1++;
	}
    }
    //end by chenfei
    ...
    {   
        cnt++;
        ret=pmic_bank1_config_interface(0x6A, 0x71, 0xFF, 0x0);
	    printk("CHENFEI FGADC SW RESET gFG_columb = %d \n",gFG_columb);        
        gFG_columb = fgauge_read_columb_reset();
        val_car = gFG_columb;
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "#");
	printk("CHENFEI FGADC RESET WHILE cnt = %d\n",cnt);
	if(cnt > 5)
	{
		gSW_rst_err = 1;
		printk("CHENFEI FGADC FG_SW_rst error \n");
		break;
	}
	gSW_rst_err = 0;	
    }
...
{    
    ...
    if(gCurrent_rst == 1 || gSW_rst_err == 1 || gFG_columb_err == 1)
    {
    ...
    //}
	//add by carl.li@jrdcom.com for PR377228
	if(gFG_current < 10)
	{	FGADC_Reset_SW_Parameter();
		FGADC_Reset_SW_Parameter();
		FGADC_Reset_SW_Parameter();

		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[FGADC] Call FGADC_Reset_SW_Parameter.\r\n");
	} //add end
...
//// platform_driver API 
///////////////////////////////////////////////////////////////////////////////////////////
static int mt6329_fgadc_probe(struct platform_device *dev)    
{
...
#else
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6329 FGADC driver probe!! \n" );
...
static int mt6329_fgadc_remove(struct platform_device *dev)    
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6329 FGADC driver remove!! \n" );

    return 0;
}

static void mt6329_fgadc_shutdown(struct platform_device *dev)    
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6329 FGADC driver shutdown!! \n" );        
}
...
static int mt6329_fgadc_suspend(struct platform_device *dev, pm_message_t state)    
{
...
static int mt6329_fgadc_resume(struct platform_device *dev)
{
#if defined(CONFIG_POWER_EXT)
//#if 0
...
struct platform_device MT6329_fgadc_device = {
        .name                = "mt6329-fgadc",
        .id                    = -1,
};

static struct platform_driver mt6329_fgadc_driver = {
    .probe        = mt6329_fgadc_probe,
    .remove        = mt6329_fgadc_remove,
    .shutdown    = mt6329_fgadc_shutdown,
    //#ifdef CONFIG_PM
    .suspend    = mt6329_fgadc_suspend,
    .resume        = mt6329_fgadc_resume,
    //#endif
    .driver         = {
        .name = "mt6329-fgadc",
    },
...
static int __init mt6329_fgadc_init(void)
{
    int ret;
    
    ret = platform_device_register(&MT6329_fgadc_device);
    if (ret) {
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6329_fgadc_driver] Unable to device register(%d)\n", ret);
        return ret;
    }
    
    ret = platform_driver_register(&mt6329_fgadc_driver);
    if (ret) {
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6329_fgadc_driver] Unable to register driver (%d)\n", ret);
        return ret;
    }

    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6329_fgadc_driver] Initialization : DONE \n");

    return 0;

}

static void __exit mt6329_fgadc_exit (void)
{
}

module_init(mt6329_fgadc_init);
module_exit(mt6329_fgadc_exit);

MODULE_AUTHOR("James Lo");
MODULE_DESCRIPTION("MT6329 FGADC Device Driver");
MODULE_LICENSE("GPL");
#endif
